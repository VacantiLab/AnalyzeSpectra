def integrate_peaks(ic_smooth_dict,peak_start_t_dict,peak_end_t_dict,peak_start_i_dict,peak_end_i_dict,x_data_numpy,fragment_dict,fragments_list):
    ...
    fragment_dict_complete = copy.deepcopy(fragment_dict)

    mzs_of_peak_starts = np.array(list(peak_start_t_dict.keys()))

    for frag_iter in fragments_list:
        mz = fragment_dict_complete[frag_iter]['mz']
        rt = fragment_dict_complete[frag_iter]['rt']
        ...
        for i in mz:
            ...
            if not peak_present:
                fragment_dict_complete[frag_iter]['areas'] = np.append(fragment_dict_complete[frag_iter]['areas'],0)
            if peak_present:
